...
def get_bbox_from_mask(mask):
    # Se for tensor, converta para numpy
...
def get_bbox_from_mask_tuple(mask):
    """
    Compute the bounding box (x, y, w, h) from a binary mask.

    :param mask: A binary mask (torch.Tensor or numpy.ndarray)
    :return: Bounding box as a tuple (x, y, w, h)
    """
    if isinstance(mask, torch.Tensor):
        mask = mask.cpu().numpy()
