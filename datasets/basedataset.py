...
from datasets.geometryutils import relative_transformation
...
class GradSLAMDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.device = device
        self.png_depth_scale = config_dict["camera"]["scale"]

        self.orig_height = config_dict["camera"]["H"]
        self.orig_width = config_dict["camera"]["W"]
        self.fx = config_dict["camera"]["fx"]
        self.fy = config_dict["camera"]["fy"]
        self.cx = config_dict["camera"]["cx"]
        self.cy = config_dict["camera"]["cy"]
        ...
        self.distortion = (
            np.array(config_dict["camera"]["distortion"])
            if "distortion" in config_dict["camera"]
            else None
        )
        self.crop_size = (
            config_dict["camera"]["crop_size"] if "crop_size" in config_dict["camera"] else None
        )

        self.crop_edge = None
        if "crop_edge" in config_dict["camera"].keys():
            self.crop_edge = config_dict["camera"]["crop_edge"]
     ...
     #   self.poses = self.load_poses()
        ...
        self.retained_inds = torch.arange(self.num_imgs)[self.start : self.end]
    ...
    def __getitem__(self, index):
        ...
        return (
                color.type(self.dtype),
                depth.type(self.dtype)
            )
