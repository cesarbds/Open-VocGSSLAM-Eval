...
class ScannetSemanticDataset(GradSLAMDataset):
    ...
    def get_filepaths(self):
        ...
        self.object_paths = natsorted(glob.glob(f"{self.input_folder}/label-filt/*.png"))

        self.embedding_paths = None
        if self.load_embeddings:
            self.embedding_paths = natsorted(glob.glob(f"{self.input_folder}/{self.embedding_dir}/*.pt"))
