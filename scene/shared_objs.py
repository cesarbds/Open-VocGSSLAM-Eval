...
class SharedPoints(nn.Module):
    ...
    def input_values(self, new_points, new_colors, new_z_values, new_filter):
        if new_points.shape[0] > self.points.shape[0]:
            raise RuntimeError(
                f"SharedPoints capacity exceeded: received {new_points.shape[0]}, "
                f"capacity is {self.points.shape[0]}"
            )
...
class SharedGaussians(nn.Module):
    ...
    def input_values(self, new_xyz, new_colors, new_rots, new_scales, new_z_values, new_trackable_filter, embeddings=None, seg_map=None):
        # on CPU memory
        if new_xyz.shape[0] > self.xyz.shape[0]:
            raise RuntimeError(
                f"SharedGaussians capacity exceeded: received {new_xyz.shape[0]}, "
    ...
    def get_values(self, include_features=False):
        if include_features:
            return  copy.deepcopy(self.xyz[:self.using_idx[0],:]),\
                    ...
                    copy.deepcopy(self.embeddings[:self.using_idx[0]])
        else:
            return  copy.deepcopy(self.xyz[:self.using_idx[0],:]),\
                    copy.deepcopy(self.colors[:self.using_idx[0],:]),\
                    copy.deepcopy(self.rots[:self.using_idx[0],:]),\
...
class SharedTargetPoints(nn.Module):
    ...
    def input_values(self, new_xyz, new_rots, new_scales):
        ...
        if new_count > self.num_points:
            raise RuntimeError(
                f"SharedTargetPoints capacity exceeded: received {new_count}, "
                f"capacity is {self.num_points}. Restart with "
                f"--target-gaussian-capacity greater than {new_count}."
            )
        self.using_idx[0] = new_count
...
class SharedCam(nn.Module):
    def __init__(self, FoVx, FoVy, image, depth_image,
                 cx, cy, fx, fy,
                 trans=np.array([0.0, 0.0, 0.0]), scale=1.0, include_feature=False):
        super().__init__()
