...
class TrajManager:
    def __init__(self, which_dataset, dataset_path, start_frame, end_frame, stride):
        ...
        if self.which_dataset == "tum":
            ...
            self.gt_poses = self.replica_load_poses(self.dataset_path + '/traj.txt')
            scene_path = Path(self.dataset_path)
            self.color_paths = [str(path) for path in sorted((scene_path / "images").glob("*.jpg"))]
            self.depth_paths = [str(path) for path in sorted((scene_path / "depth_images").glob("*.png"))]
        else:
