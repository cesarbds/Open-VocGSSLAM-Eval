...
def crop_all_bounding_boxs(image, masks, block_background=False, bbox_margin=0, which_sam='sam'):
    """
    Draws and crop bounding boxs on the given image using the provided XYWH format bbox.
    ...
    for mask in masks:
        ...
        if seg is not None:
            ...
            if seg.max() <= 1:
                seg = seg * 255  # converte de máscara booleana (0/1) para 0/255
        if block_background:
            crop = crop_image(image, mask)
        else:
            crop = crop_bbox(image, mask["bbox"], bbox_margin)
        if isinstance(crop, torch.Tensor):
            # ToPILImage interprets tensors as CxHxW but NumPy arrays as
            # HxWxC; crop_image returns the latter layout.
            crop = crop.cpu().numpy()
        crop = resize(ToPILImage()(crop))
        images.append(crop)
    return images
...
def crop_image(image, mask):
    ...
    mask_torch = torch.as_tensor(mask["segmentation"], device=image_torch.device)
    masked = image_torch * mask_torch.unsqueeze(-1)
...
def crop_bbox(image, bbox, bbox_margin=0):
    ...
    # make x, y, w, h positive and 'no't bigger than the image size and int
    x, y, w, h = int(x), int(y), int(w), int(h)
