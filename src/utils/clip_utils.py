...
def get_img_feats(img, preprocess, clip_model):
    """
    Get image features using a half-precision CLIP model.
    Ensures the input matches model dtype (fp16) and device.
    """
    ...
    with torch.no_grad():
            img_in = img_in.to(device="cuda")
            img_feats = clip_model.encode_image(img_in)
            img_feats = torch.nn.functional.normalize(img_feats, dim=-1)
    # Convert to half at the very end
    img_feats = img_feats

    return img_feats.cpu().numpy().astype(np.float16)
