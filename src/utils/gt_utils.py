...
# This file is part of GS_ICP_SLAM.
#
...
# Add the project root to path (two levels up from src/utils/)
sys.path.append(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))

# Now import using absolute import from project root
...
class GT_Manager:
    def __init__(self, which_dataset, dataset_path, end_frame, relative_pose=True):
        '''
        ...
        elif self.which_dataset in ("replica", "scannet"):
            print(self.dataset_path + '/traj.txt')
    ...
    def replica_load_poses(self, path):
        poses = []
        print("Loading GT poses from ", path)
        ...
        for i in range(len(lines)):
            ...
            poses.append(c2w)
        poses = poses[self.start : self.end : self.stride]
        poses = torch.stack(poses)

        # if self.relative_pose: # True
        #     self.transformed_poses = self._preprocess_poses(poses)
        # else:
        self.transformed_poses = poses
        return np.array(self.transformed_poses)
